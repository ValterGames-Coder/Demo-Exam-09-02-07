...
class MainAppWindow(tk.Tk):
    ...
    def _create_layout(self):
        ...
        logo_label = ttk.Label(top_frame, text=config.APP_NAME, font=config.FONT_TITLE)
        if os.path.exists(config.LOGO_PATH):
            try:
                logo_img = self._resize_image(config.LOGO_PATH, config.LOGO_SIZE)
                if logo_img:
    ...
    def _populate_controls(self):
        ...
        if self.current_view == 'products':
            if is_privileged:
                ...
                sort_frame_inner = ttk.Frame(left_frame)
                sort_frame_inner.pack(side='left')
    ...
    def populate_content(self, *args):
        ...
        if self.current_view == 'products':
            ...
            if hasattr(self, 'search_var'):
                filters['search_query'] = self.search_var.get()
            if hasattr(self, 'supplier_var'):
                filters['supplier'] = self.supplier_var.get()
    ...
    def _create_product_card(self, parent, product):
        is_admin = self.user_data['role_name'] == 'Администратор'
        has_discount = product.get('current_discount', 0) > 15
        card_style = "Discount.Card.TFrame" if has_discount else "Card.TFrame"
        child_style_prefix = "Discount." if has_discount else ""
        command = (lambda e, p=product: self.open_product_editor(p)) if is_admin else None

        card = ttk.Frame(parent, style=card_style)
        card.pack(pady=5, fill='x', expand=True)

        img_label = ttk.Label(card, style=f"{child_style_prefix}TLabel")
        img = self._resize_image(product.get('image_path') or config.PLACEHOLDER_IMAGE_PATH, config.PRODUCT_IMAGE_SIZE)
        if img:
            img_label.configure(image=img)
            self.image_references.append(img)
        img_label.pack(side='left', padx=10, pady=10)

        center = ttk.Frame(card, style=f"{child_style_prefix}TFrame")
        center.pack(side='left', expand=True, fill='both', pady=10)
        ...
        details = (
            ...
            f"Цена: {product.get('price', 0):.2f} руб.\n"
            f"Кол-во: {product.get('stock_quantity', 0)} {product.get('unit', 'шт.')}"
        )
        ttk.Label(center, text=details, font=config.FONT_CARD_BODY, justify='left', style=f"{child_style_prefix}TLabel").pack(anchor='w')

        right = ttk.Frame(card, width=130, style=f"{child_style_prefix}TFrame")
        right.pack(side='right', fill='y', padx=10, pady=10)
        right.pack_propagate(False)
        ttk.Label(right, text="Действующая\nскидка", justify='center', font=config.FONT_CARD_BODY, style=f"{child_style_prefix}TLabel").pack(expand=True)
        ttk.Label(right, text=f"{product.get('current_discount', 0)}%", font=config.FONT_CARD_ACCENT, style=f"{child_style_prefix}Accent.TLabel").pack(expand=True)
    ...
    def _create_order_card(self, parent, order):
        ...
        ttk.Label(right, text=str(order.get('delivery_date', 'N/A')), font=config.FONT_CARD_ACCENT).pack(expand=True)
